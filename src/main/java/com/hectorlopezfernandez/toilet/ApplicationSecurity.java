...
public class ApplicationSecurity extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests().antMatchers(MANAGEMENT_ENDPOINTS).hasAuthority(WellKnownRoles.ADMIN.name());
		// app access rules
		http.requiresChannel().antMatchers("/login","/logout","/login.page","/admin/*.page","/admin/api/**").requiresSecure();
		http.authorizeRequests().antMatchers("/admin/*.page","/admin/api/**").hasAuthority(WellKnownRoles.ADMIN.name());
